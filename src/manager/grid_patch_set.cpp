...
bool grid_patch_set::load(std::istream& file)
{
...
bool grid_patch_set::load(const std::filesystem::path& maps)
{
	io::bittable_serialize S;
...
int grid_patch_set::deserialize(io::bittable_serialize& S, int max_grids, uint32_t flags)
{
	// read header
	if (S.get_type() == io::bittable_type::NONE) {
		// header not read
		if ((flags & SKIP_HEADER) != 0) {
			WARTHOG_GWARN("grid patch SKIP_HEADER for serializer when header has not been read.");
			return -1;
		}
		if (auto ec = S.read_header(); ec != std::errc{}) {
			WARTHOG_GWARN_FMT("grid patch failed to read header code={}", (int)ec);
			return -1;
		}
	} else {
		// header has been read, check
		if ((flags & FORCE_HEADER) != 0) {
			WARTHOG_GWARN("grid patch FORCE_HEADER for serializer when header has already been read.");
			return -1;
		}
	}
	auto type = S.get_type();
	if (type != io::bittable_type::OCTILE && type != io::bittable_type::PATCH) {
			WARTHOG_GWARN("grid patch only supports reading type octile or patch.");
			return -1;
	}
	...
	int count = 0;
	if (max_grids < 0) {
		max_grids = std::numeric_limits<int>::max();
	}
	while (count < max_grids && S.get_patch_count() < S.get_patch_amount()) {
		// read new grid
		++count;
		if (auto ec = S.read_grid_header(); ec != std::errc{}) {
			WARTHOG_GWARN_FMT("grid patch failed to read patch {} header code={}", count, (int)ec);
			return -count;
		}
		if ((flags & IGNORE_INDEX) == 0) {
			...
			if (S.get_patch_id() != patches_.size()) {
				// index mismatch
				WARTHOG_GWARN_FMT("grid patch failed patch {} index mismatch of {} expected {}", count, S.get_patch_id(), patches_.size());
				return -count;
			}
		}
		// setup patch data
		...
		auto* grid_data = static_cast<bittable::value_type*>(grid_res_.allocate(bytes, alignof(bittable::value_type)));
		bittable patch(grid_data, dim.width, dim.height);
		if (auto ec = S.read_grid_data(patch); ec != std::errc{}) {
			WARTHOG_GWARN_FMT("grid patch failed to read patch {} data code={}", count, (int)ec);
			return -count;
		}
...
bool grid_patch_set::push_copy(bittable table, uint16_t offset_x, uint16_t offset_y,
...
bool grid_patch_set::push_ref(bittable patch)
{
	patches_.push_back(patch);
	return true;
}

void grid_patch_set::reset()
{
	patches_.clear();
	grid_res_.release();
}
