...
scenario_serialize::scenario_serialize() : m_line(static_cast<char*>(m_dyn_res.allocate(max_line_length+2))),
	m_unget_line(&m_dyn_res),
...
std::errc scenario_serialize::open_read(std::istream* scenario)
{
	close();
	if (scenario != nullptr)
	{
		m_scenario_in = scenario;
	} else {
...
void scenario_serialize::unreadline(std::string_view line)
{
	m_unget_line = line;
}

void scenario_serialize::set_relative_map_filename(const std::filesystem::path& filename)
{
	m_map_filename = std::filesystem::proximate(filename, m_scenario_filename);
}

std::istream& scenario_serialize::line_stream(std::string_view line)
{
	m_iss.clear();
...
std::errc scenario_serialize::read_header_v1(std::istream* in)
{
	...
	if (s != final)
	{
		if (Q.width <= 0 || Q.width > GRID_MAX_SIZE || Q.height <= 0 || Q.height > GRID_MAX_SIZE)
		{
			m_state = serialize_state::error;
			return std::errc::invalid_argument;
		}
...
auto scenario_serialize::read_query_line_v1(scenario_query& query, std::istream* in) -> std::pair<query_res,std::errc>
{
	assert(can_read(in));
	...
	assert(!std::all_of(line.begin(), line.end(), [](char a) { return std::isspace((unsigned char)a); }));
	auto& iss = line_stream(line);
	if (!(iss >> query.bucket >> query.map >> query.width >> query.height >> query.start_x >> query.start_y >> query.goal_x >> query.goal_y >> query.dist[0]))
	{
		return {invalid, std::errc::io_error};
	}
	...
	if (m_force_int) {
		...
		if (rint(query.start_x) != query.start_x || rint(query.start_y) != query.start_y
			|| rint(query.goal_x) != query.goal_x || rint(query.goal_y) != query.goal_y)
		{
			return {invalid, std::errc{}};
		}
...
auto scenario_serialize::read_query_line_v2(scenario_query& query, std::istream* in) -> std::pair<query_res,std::errc>
{
	assert(can_read(in));
	int width, height;
	double d;
...
auto scenario_serialize::read_patch_line_v2(scenario_patch& patch, std::istream* in) -> std::pair<query_res,std::errc>
{
	assert(can_read());
	int width, height;
	double d;
