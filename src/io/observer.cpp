...
void
stream_observer::stream(std::ostream& stream)
{
	stream_        = &stream;
	shared_stream_ = nullptr;
}
void
stream_observer::stream_share(const shared_stream_t& stream)
{
	shared_stream_ = stream;
	stream_        = shared_stream_.get();
}
void
stream_observer::stream_share(const stream_observer& stream)
{
	shared_stream_ = stream.shared_stream_;
	stream_        = stream.stream_;
}
void
stream_observer::stream_stdout()
{
	shared_stream_ = nullptr;
	stream_        = &std::cout;
}
void
stream_observer::stream_stderr()
{
	shared_stream_ = nullptr;
	stream_        = &std::cerr;
}
void
stream_observer::clear_stream()
{
	shared_stream_ = nullptr;
	stream_        = nullptr;
}
...
void
posthoc_trace::print_posthoc_header()
{
	if(*this)
	{
