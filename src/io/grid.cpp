...
bittable_serialize::read_header(std::istream* in)
{
	...
	{
		parser par(line);
		if(!par.next(token) || token != "type") { return std::errc::io_error; }
...
bittable_serialize::read_grid_header(std::istream* in)
{
	...
	if(m_type == bittable_type::PATCH)
	{
		...
		if(token != "patch") { return std::errc::argument_out_of_domain; }
	...
	if(err != std::errc{}) { return err; }
	{
		parser par(line);
		if(!par.next(token).eof()) { return std::errc::io_error; }
		if(token != "map") { return std::errc::argument_out_of_domain; }
	}
