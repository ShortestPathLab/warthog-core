...
bittable_serialize::read_header(std::istream& in)
{
	...
	{
		if(std::strcmp(buffer, "patches") != 0) return false;
		in >> m_patch_count;
		if(!read_key()) return false;
	}

	if(std::strcmp(buffer, "height") != 0) return false;
	if(!(in >> m_dim.height)) return false;
	if(!read_key()) return false;
	if(std::strcmp(buffer, "width") != 0) return false;
	if(!(in >> m_dim.width)) return false;
	m_type = detected_type;
	if(!read_key() || std::strcmp(buffer, "map") != 0) return false;
	return true;
}
