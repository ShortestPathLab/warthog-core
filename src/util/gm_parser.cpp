...
gm_parser::gm_parser(const char* filename)
{
...
void
gm_parser::parse_header(std::fstream& mapfs)
{
	// read header fields
	...
	for(int i = 0; i < 3; i++)
	{
		...
		{
			mapfs >> hvalue;
			if(mapfs.good()) { contents[hfield] = hvalue; }
			else
			{
		...
		{
			std::cerr << "err;  map load failed. format looks wrong."
			          << std::endl;
			exit(1);
		}
	}
	...
	this->header_.height_
	    = (uint32_t)atoi(contents[std::string("height")].c_str());
	...
	this->header_.width_
	    = (uint32_t)atoi(contents[std::string("width")].c_str());
...
void
gm_parser::parse_map(std::fstream& mapfs)
{
	std::string hfield;
	mapfs >> hfield;
	...
	if(index != max_tiles)
	{
		std::cerr << "err; expected " << max_tiles << " tiles; read " << index
		          << " tiles." << std::endl;
		exit(1);
	}
}
