...
bool
test_bittable_vector(const bt& b, const vec& v)
{
	for(size_t y = 0, i = 0; y < b.height(); ++y)
		for(size_t x = 0; x < b.width(); ++x, ++i)
		{
			if(b.xy_to_id(x, y) != i) return false;
			if(b.get(i) != v[y][x]) return false;
		}
	return true;
}
    ...
    (uint32_t, 1), (uint32_t, 16), (uint64_t, 32), (uint64_t, 64))
{
	...
	constexpr T value_one = []() {
		if constexpr(B >= sizeof(T) * CHAR_BIT) { return value_one_word; }
	...
	SECTION("WHOLE")
	{
		...
		REQUIRE(std::all_of(
		    ...
		    [](T v) noexcept { return v == value_zero; }));
		table.flip();
		REQUIRE(std::all_of(
		    element_data.begin(), element_data.end(),
		    [](T v) noexcept { return v == value_one_word; }));
		...
		{
			std::generate(
			    element_data.begin(), element_data.end(), std::ref(rng));
			table.fill(value_alt);
			REQUIRE(std::all_of(
			    element_data.begin(), element_data.end(),
			    [](T v) noexcept { return v == value_alt_word; }));
			table.flip();
			REQUIRE(std::all_of(
			    element_data.begin(), element_data.end(),
			    [](T v) noexcept { return v == (T)~value_alt_word; }));
		}
	}
	...
	SECTION("OPERATORS")
	{
		...
		for(size_t i = 0, ie = width * height; i < ie; ++i)
		{
			...
			default:
				table.bit_neg(pos);
				valuetable[y][x]
				    = ~valuetable[y][x] & bittable::value_bit_mask;
				break;
			}
		}
		CHECK(test_bittable_vector(table, valuetable));
