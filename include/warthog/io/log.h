...
// Logging utility.
//
// The log_sink is a non-owning copyable struct that points to the data and logging function calls.
// All logging is performed through log_sink.
// All classes here are thread safe, follow comments for outlaying cases.
//
...
// The WARTHOG_TRACE and others in this header provide interfaces to log to a special
// logger<> class.  This logger<> class knows at compile time the minimum level to log,
// and with the use of a macro will be compiled out if that level was not set at compile time
// when used with the logging macros.
//
// The global logger can be acquired with the glog() (logger<>) or glogs() (log_sink), and set with
// set_glog(log_sink).  The log level of the global logger is set though the WARTHOG_LOG definition,
// with a default of 1(debug) for debug builds, and 2(information) for release (NDEBUG defined).
// Macros like WARTHOG_GTRACE will automatically write to the global logger.
//
// The _IF will only log if a runtime if true.
// The _FMT uses the std::format from C++20 to format the output messages.
// The logging utilities uses dynamic memory std::strings to produce the final log message strings,
// thus is better to be disabled for release builds through the log level.
//
// Log messages produce a messaged as "[TIME LEVEL] msg".
// The time is formatted in ISO with space (yyyy-mm-dd hh:mm:ss), but can be overridden with
// define cstring WARTHOG_LOG_TIME.
// Clock is in local system time, but can be set to UTC by definiong WARTHOG_LOG_CLOCK_UTC.
//
...
///
/// Is thread safe as long as std::ios_base::sync_with_stdio(false) has not been called.
/// Can copy go log_sink and then be destructed without issue.
...
/// This class is handled specially by the logging macros to only log if log_level
/// is a min_level.
/// When setting a long_sink, will clear all pointers below MinLevel.
...
/// @brief get the global logger, default is set to std::cerr through log_sink_std
...
struct global_logger : logger<MinLevel>
{
	...
	/// @brief Updates the sink to the new glog sink.  Must be called if set_glog was used.
	void resync_global()
	{
