...
/// @brief string_view is either empty or only contains spaces (as
/// std::isspace)
inline bool
is_blank(std::string_view str)
{
...
inline token_return
get_token_quoted(std::string_view str, char quote = '"', char escape = '\\')
{
	if(quote == escape) { return {std::string_view(), 0}; }
	auto fstr = ltrim(str);
	if(fstr.empty()) { return {std::string_view(), str.size()}; }
	token_return ret;
	ret.trim = str.size() - fstr.size();
...
inline std::errc
parse_token(std::string_view token, std::integral auto& out, int base = 10)
{
	const char* const f  = token.data();
	const char* const ed = token.data() + token.size();
	auto res             = std::from_chars(token.data(), ed, out, base);
	if(res.ec != std::errc{}) return res.ec;
	// value set
	if(res.ptr != ed) { return std::errc::invalid_argument; }
	return std::errc{};
}
inline std::errc
parse_token(
    std::string_view token, std::floating_point auto& out,
...
inline std::pair<std::string_view, std::errc>
parse_token_part(
    std::string_view token, std::integral auto& out, int base = 10)
{
	std::from_chars_result res = std::from_chars(
	    token.data(), token.data() + token.size(), out, base);
	if(res.ec != std::errc{}) return {{}, res.ec};
	// value set
...
parse_token_quoted(
    std::string_view token, char* buffer, size_t size, char quote = '"',
    char escape = '\\')
{
	if(quote == escape) { return {nullptr, std::errc::invalid_argument}; }
	...
	if(token[0] != '"')
	{
		...
		if(std::any_of(token.data(), ed, [](char c) {
	...
	while(true)
	{
		...
		else if(*t_at == '"') [[unlikely]]
		{
			...
			*(buffer++) = '\0';
			...
			return {
			    buffer,
			    t_at + 1 == ed ? std::errc{} : std::errc::invalid_argument};
		}
		*(buffer++) = *(t_at++);
		if(--size == 0)
		{
...
inline std::errc
parse_token_quoted(
    std::string_view token, auto& str_out, char quote = '"',
    char escape = '\\')
{
	str_out.resize(token.size() + 1); // plus 1 to hold \0 placed in buffer
	auto [ed, ec] = parse_token_quoted(
	    token, str_out.data(), str_out.size(), quote, escape);
	if(ec != std::errc{})
	{
...
public:
	...
	operator bool() const noexcept { return m_error == std::errc{}; }
	...
	std::string_view
	last_token() const noexcept
	{
		return m_token;
	}

	string_parser&
	next(std::integral auto& int_out, int base = 10)
	{
		if(is_error()) // do nothing if in error state
			return *this;
		if(!next_token_()) return *this;
		std::errc ec = util::parse_token(m_token, int_out, base);
		if(ec != std::errc{}) m_error = ec;
		return *this;
	}
	string_parser&
	next(
	    std::floating_point auto& float_out,
	    std::chars_format fmt = std::chars_format::general)
	{
		if(is_error()) // do nothing if in error state
			return *this;
	...
	string_parser&
	next(std::string_view& out_token)
	{
		if(is_error()) // do nothing if in error state
			return *this;
		if(!next_token_()) return *this;
	...
	string_parser&
	next_q(std::span<char>& buffer, char quote = '"', char escape = '\\')
	{
		if(is_error()) // do nothing if in error state
			return *this;
		if(buffer.empty())
		...
		if(!next_token_quoted_(quote, escape)) return *this;
		auto [ed, ec] = util::parse_token_quoted(
		    m_token, buffer.data(), buffer.size(), quote, escape);
		if(ec != std::errc{})
		{
			m_error = ec;
			return *this;
	...
	string_parser&
	next_q(std::string& out_token, char quote = '"', char escape = '\\')
	{
		if(is_error()) // do nothing if in error state
			return *this;
		if(!next_token_quoted_(quote, escape)) return *this;
		std::errc ec
		    = util::parse_token_quoted(m_token, out_token, quote, escape);
		if(ec != std::errc{})
		{
			m_error = ec;
			return *this;
	...
	string_parser&
	next_q(std::pmr::string& out_token, char quote = '"', char escape = '\\')
	{
		if(is_error()) // do nothing if in error state
			return *this;
		if(!next_token_quoted_(quote, escape)) return *this;
	...
	string_parser&
	ignore_q(char quote = '"', char escape = '\\')
	{
		if(is_error()) // do nothing if in error state
			return *this;
...
protected:
	...
	bool
	next_token_quoted_(char quote, char escape)
	{
