...
[[deprecated("TDB")]]
std::ostream&
operator<<(std::ostream& str, const std::unordered_set<T>& v)
{
	str << "[ ";
...
[[deprecated("TDB")]]
std::ostream&
operator<<(std::ostream& stream, const std::unordered_map<K, V>& map)
{
	stream << "{ ";
	for(auto el : map)
	{
...
[[deprecated("TDB")]]
std::ostream&
operator<<(std::ostream& str, const std::vector<T>& v)
{
	str << "[ ";
...
[[deprecated("TDB")]]
void
save_vector(std::FILE* file, const std::vector<T>& v)
{
	int s = v.size();
...
load_vector(std::FILE* file)
{
	...
	{
		WARTHOG_GERROR_FMT(
		    "we were expecting to read {} but we read {} elements instead", s,
		    stuffRead);
		throw std::runtime_error("std::fread failed");
	}
