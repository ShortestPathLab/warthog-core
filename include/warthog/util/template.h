...
struct for_each_integer_sequence<std::integer_sequence<IST, Values...>>
{
	...
	{
		if (Arg0 == value) {
			return static_cast<Ret>(tfunc(std::integral_constant<IST, Arg0>()));
		} else {
...
{
	details::for_each_integer_sequence<IST>::template apply_if<void>(value, std::forward<TemplateFunc>(tfunc));
}
template <typename Ret, typename IST, typename TemplateFunc>
void choose_integer_sequence(auto value, TemplateFunc&& tfunc)
{
	details::for_each_integer_sequence<IST>::template apply_if<Ret>(value, std::forward<TemplateFunc>(tfunc));
}
