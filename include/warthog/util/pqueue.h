#ifndef WARTHOG_UTIL_PQUEUE_H
#define WARTHOG_UTIL_PQUEUE_H
...
template<class Comparator = search::cmp_less_search_node, class QType = min_q>
class pqueue
{
public:
	pqueue(Comparator* cmp, unsigned int size = 1024) : pqueue(size)
	{
		cmp_ = cmp;
	}

	pqueue(unsigned int size = 1024)
	    : maxsize_(size), minqueue_(QType().is_min_), queuesize_(0), elts_(0),
	      heap_ops_(0)
	{
		resize(size);
	}
	...
	void
	decrease_key(search::search_node* val)
	{
		assert(val->get_priority() < queuesize_);
		minqueue_ ? heapify_up(val->get_priority())
		          : heapify_down(val->get_priority());
	}

	void
	increase_key(search::search_node* val)
	{
		assert(val->get_priority() < queuesize_);
		minqueue_ ? heapify_down(val->get_priority())
		          : heapify_up(val->get_priority());
	}

	// add a new element to the pqueue
	void
	push(search::search_node* val)
	{
	...
	pop()
	{
		if(queuesize_ == 0) { return 0; }

		search::search_node* ans = elts_[0];
		queuesize_--;
	...
	inline bool
	contains(search::search_node* n)
	{
		unsigned int index = n->get_priority();
		if(index < queuesize_ && &*n == &*elts_[index]) { return true; }
		return false;
	}

	// retrieve the top element without removing it
	inline search::search_node*
	peek()
	{
		if(queuesize_ > 0) { return this->elts_[0]; }
		return 0;
	}
	...
	mem()
	{
		return maxsize_ * sizeof(search::search_node*) + sizeof(*this);
	}
...
using pqueue_min = pqueue<search::cmp_less_search_node, min_q>;
using pqueue_max = pqueue<search::cmp_greater_search_node, max_q>;

}

#endif // WARTHOG_UTIL_PQUEUE_H
