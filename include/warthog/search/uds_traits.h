...
{
	if constexpr (requires { { Traits::ac } -> std::same_as<admissibility_criteria>; }) {
		return Traits::ac;
	} else {
...
{
	if constexpr (requires { { Traits::fc } -> std::same_as<feasibility_criteria>; }) {
		return Traits::fc;
	} else {
...
{
	if constexpr (requires { { Traits::rp } -> std::same_as<reopen_policy>; }) {
		return Traits::rp;
	} else {
