...
public:
	...
	gridmap_slider
	get_neighbours_slider(pad_id grid_id)
	{
...
struct gridmap_slider
{
	...
	get_neighbours_64bit_le() const noexcept
	{
		...
		std::memcpy(&return_value[0], loc - width8, sizeof(uint64_t));
		std::memcpy(&return_value[1], loc, sizeof(uint64_t));
