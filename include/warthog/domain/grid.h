...
typedef enum : uint8_t
{
	...
	SOUTHEAST_ID, // 0b110
	SOUTHWEST_ID, // 0b111
	// only function with `secic` will consider the following
	// second-intercardinal dirctions
...
static_assert(
    NORTH_ID < 4 && SOUTH_ID < 4 && EAST_ID < 4 && WEST_ID < 4,
    "cardinals id must be less than 4");
static_assert(
    NORTHEAST_ID >= 4 && NORTHWEST_ID >= 4 && SOUTHEAST_ID >= 4
        && SOUTHWEST_ID >= 4,
    "intercardinals id must be at least 4");
static_assert(
    NORTHEAST_ID < 8 && NORTHWEST_ID < 8 && SOUTHEAST_ID < 8
        && SOUTHWEST_ID < 8,
...
{
	direction_id c
	    = static_cast<direction_id>(static_cast<uint8_t>(d) & 0b000'111);
...
inline std::ostream&
operator<<(std::ostream& out, direction_id d)
{
	constexpr const char* OUTCHAR[8]
	    = {"N", "S", "E", "W", "NE", "NW", "SE", "SW"};
	if(d < 8) { out << OUTCHAR[d]; }
	else { out << static_cast<uint32_t>(d); }
	return out;
}
...
dir_intercardinal_hori(direction_id d) noexcept
{
	...
	if constexpr(
	    NORTHEAST_ID == 4 && NORTHWEST_ID == 5 && SOUTHEAST_ID == 6
	    && SOUTHWEST_ID == 7)
	{
		// precise ordering, use math method
		if constexpr(WEST_ID == EAST_ID + 1)
		{ // EAST_ID = 2, WEST_ID = 3
			return static_cast<direction_id>(EAST_ID + (d & 0b001));
		}
		else { return (d & 0b001) ? WEST_ID : EAST_ID; }
	}
	else
	{
...
dir_intercardinal_vert(direction_id d) noexcept
{
	...
	{
		// precise ordering, use math method
		if constexpr(SOUTH_ID == NORTH_ID + 1)
		{ // EAST_ID = 2, WEST_ID = 3
			return static_cast<direction_id>(NORTH_ID + ((d & 0b010) >> 1));
		}
		else { return (d & 0b010) ? SOUTH_ID : NORTH_ID; }
	}
	else
	{
...
{
	assert(static_cast<uint8_t>(d) < 8);
	constexpr uint64_t sel = ((uint64_t)uint8_t(0u) << (NORTH_ID << 3))
	...
	return static_cast<int32_t>(
	    static_cast<int8_t>(static_cast<uint8_t>(sel >> (d << 3))));
}
...
point_signed_diff(point a, point b) noexcept
{
	return {
	    static_cast<int32_t>(
	        static_cast<int32_t>(b.x) - static_cast<int32_t>(a.x)),
	    static_cast<int32_t>(
...
dir_unit_point(direction_id d) noexcept
{
	...
	constexpr uint32_t packed_reldir = (0b0001 << NORTH_ID * 4)
	    | (0b1001 << SOUTH_ID * 4) | (0b0110 << EAST_ID * 4)
	    | (0b0100 << WEST_ID * 4) | (0b0010 << NORTHEAST_ID * 4)
...
point_to_direction_id(point p1, point p2) noexcept
{
	...
	if(c.p.x == 0) { return c.p.y >= 0 ? SOUTH_ID : NORTH_ID; }
	else if(c.p.y == 0) { return c.p.x >= 0 ? EAST_ID : WEST_ID; }
	else
	{
